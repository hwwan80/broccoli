      ...
      treeDirPromise = RSVP.resolve()
        .then(function () {
          return tree.read(function readTree (subtree) {
            ...
            // Start subtree timer
            var subtreeStartTime = process.hrtime();
            ...
            return RSVP.resolve()
              ...
              .finally(function () {
                ...
                var now = process.hrtime();
                // Stop subtree timer
                subtreeTime += (now[0] - subtreeStartTime[0]) * 1e9 + (now[1] - subtreeStartTime[1])
        ...
        .then(function (dir) {
          ...
          var now = process.hrtime()
          node.selfTime += (now[0] - selfStartTime[0]) * 1e9 + (now[1] - selfStartTime[1]) - subtreeTime
